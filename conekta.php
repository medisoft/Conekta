...
require_once("modules/gateways/conekta/vendor/conekta/conekta-php/lib/Conekta.php");
$gateway = getGatewayVariables("conekta");
...
{
    mail($gateway['problememail'], "Conekta " . $error_type . " Error", "Conekta payment processor failed processing a charge due to the following " . $error_type . " error: " . $error_contents);
}
...
$smartyvalues["conekta_pubkey"] = $pubkey;
...
//                    echo "<pre>" . print_r($charge, true) . "</pre>";
                ...
                } catch (Conekta_ApiConnectionError $e) {
                    $error = $e->getMessage();
                    $smartyvalues["processingerror"] = 'Error: ' . $error . '.';
            ...
            if ($_POST['conektaTokenId'] != "") {
                ...
                $conekta_plan_name = "Invoice #" . $smartyvalues['invoiceid'] . ' - ' . $ng_plan_name . ' - ' . $email;

                // Create "custom" plan for this user
                try {
                    \Conekta\Conekta_Plan::create(array(
                        "amount" => $amount_subscribe_cents,
                        "interval" => "month",
                        "name" => $conekta_plan_name,
                        "currency" => "mxn",
                        "id" => $ng_plan_id
                    ));


                    // Find out if this customer already has a paying item with conekta and if they have a subscription with it
                    $current_uid = $_SESSION['uid'];
                    $q = mysql_query("SELECT subscriptionid FROM tblhosting WHERE userid='" . $current_uid . "' AND paymentmethod='conekta' AND subscriptionid !=''");
                    if (mysql_num_rows($q) > 0) {
                        $data = mysql_fetch_array($q);
                        $conekta_customer_id = $data[0];
                    } else {
                        $conekta_customer_id = "";
                    }

                    if ($conekta_customer_id == "") {
                        $customer = \Conekta\Conekta_Customer::create(array( // Sign them up for the requested plan and add the customer id into the subscription id
                            "card" => $token,
                            "plan" => $ng_plan_id,
                            "email" => $email
                        ));
                        ...
                        $c = \Conekta\Conekta_Customer::retrieve($conekta_customer_id);
...
$templatefile = "clientareacreditcard-conekta";
